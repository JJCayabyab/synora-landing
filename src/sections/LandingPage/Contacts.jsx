    ...
    <Container className='flex flex-col-reverse justify-center gap-5 md:gap-15
      ...
      <div className='md:w-1/2 gap-5'>
        ...
        >
          ...
          <button type="button" className="text-white bg-gradient focus:ring-4  focus:outline-none focus:ring-purple-200 dark:focus:ring-purple-800 font-medium rounded-lg text-sm font-heading px-5 py-2.5 text-center me-2 mb-2 transition-all duration-500">Send Message</button>

        </motion.div>
      </div>

      <div className='flex flex-col div md:w-1/2 items-center lg:items-start
                    md:flex-col'>
        <div className='text-center r md:text-left'>
          <motion.h1
            initial={{ opacity: 0, y: 50 }}
            whileInView={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, ease: 'easeOut' }}
        ...
        <div className='flex  flex-col gap-5'>
          {contactData.map((contact, index) => (
            ...
            >
              <img src={darkMode ? contact.iconDark : contact.icon}
                alt={contact.text} className="w-6 h-6" />
